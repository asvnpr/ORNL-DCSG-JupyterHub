...
def launch_docker_kernel(kernel_id, response_addr, spark_context_init_mode):
    ...
    if swarm_mode:
        ...
        user_conda_envs_dir = os.environ.get('USER_ENV_DIR')
        # TODO: need way to confirm KERNEL_USERNAME is present otherwise everything will fail 
        kernel_user = os.environ.get('KERNEL_USERNAME', 'test')
        kernels_dir = os.environ.get('USER_KERNELS_DIR')
        user_env_dir = os.path.join(user_conda_envs_dir, "jupyterhub-user-{}".format(kernel_user))
        # NOTE: should maybe change to env var in case image conda_dir changes
        container_env_dir = '/opt/conda/envs'
        volumes = {
            kernels_dir: {'bind': kernels_dir, 'mode': 'ro'},
            user_env_dir: {'bind': container_env_dir, 'mode': 'ro'}
        }
        ...
        kwargs['volume_driver'] = 'local-persist'
