...
RUN  $CONDA_DIR/bin/pip install "nb2kg==${NB2KG_VERSION}" && $CONDA_DIR/bin/conda clean --all -y && fix-permissions $CONDA_DIR && \
    fix-permissions /home/$NB_USER && $CONDA_DIR/bin/jupyter serverextension enable --py nb2kg --sys-prefix
